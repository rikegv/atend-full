...
async function seed() {
  ...
  // Planos
  await ingestContent(seedProvider, tenant!.id, "plano",
    "Plano Basic: R$89/mês. Inclui acesso à musculação e área de cardio. Sem limite de horário.");
  await ingestContent(seedProvider, tenant!.id, "plano",
    "Plano Gold: R$149/mês. Inclui musculação, cardio e todas as aulas coletivas (spinning, funcional, yoga). Sem limite de horário.");
  await ingestContent(seedProvider, tenant!.id, "plano",
    "Plano Premium: R$249/mês. Inclui tudo do Gold + 2 sessões de personal trainer por semana + acesso ao spa.");

  // FAQs
  await ingestContent(seedProvider, tenant!.id, "faq",
    "Horário de funcionamento: Segunda a sexta das 6h às 23h. Sábados das 8h às 18h. Domingos e feriados das 8h às 14h.");
  await ingestContent(seedProvider, tenant!.id, "faq",
    "Day-use: Oferecemos day-use por R$35. Inclui acesso a todas as áreas por um dia. Sujeito a disponibilidade.");
  await ingestContent(seedProvider, tenant!.id, "faq",
    "Personal trainer: Sessões avulsas de personal custam R$80/hora. No plano Premium, 2 sessões semanais já estão inclusas.");
  await ingestContent(seedProvider, tenant!.id, "faq",
    "Cancelamento: O cancelamento pode ser feito com 30 dias de antecedência. Não há multa após o período de fidelidade de 3 meses.");
  await ingestContent(seedProvider, tenant!.id, "faq",
    "Estacionamento: Estacionamento gratuito para alunos mediante apresentação da carteirinha. Vagas limitadas.");

  // Regras (guardrails)
  await ingestContent(seedProvider, tenant!.id, "regra",
    "Nunca ofereça desconto fora da tabela de preços oficial. Não crie promoções ou condições especiais por conta própria.");
  await ingestContent(seedProvider, tenant!.id, "regra",
    "Nunca prometa resultado de emagrecimento ou ganho muscular com prazo definido. Resultados variam de pessoa para pessoa.");

  // Tom de voz
  await ingestContent(seedProvider, tenant!.id, "tom",
    "Seja amigável e profissional. Use linguagem acolhedora mas objetiva. Chame o cliente pelo nome quando possível. Evite gírias e linguagem muito informal.");
